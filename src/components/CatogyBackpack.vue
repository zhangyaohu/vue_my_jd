<template>
    <div style="width:100%;height:100%;overflow: hidden">
        <div></div>
        <div></div>
        <ul class="phone_detail_container" v-if="$store.state.carts.cartsByBackPack.length>0">
            <li v-for="list in $store.state.carts.cartsByBackPack" class="phone_detail">
                <a>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            Req.getCartBackPack()
                .then((resp)=>{
                    self.$store.dispatch('getCartBackPack', resp.data)
                },()=>{
                    self.$store.dispatch('getCartBackPack', [])
                })
        }
    }
</script>
