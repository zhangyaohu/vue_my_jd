<template>
    <div style="width:100%;height:100%;overflow: hidden">
        <div></div>
        <div></div>
        <ul class="phone_detail_container" v-if="$store.state.carts.cartsByBread.length>0">
            <li v-for="list in $store.state.carts.cartsByBread" class="phone_detail">
                <a>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            Req.getCartBread()
                .then((resp) => {
                  self.$store.dispatch('getCartBread', resp.data)
                })
        }
    }
</script>
