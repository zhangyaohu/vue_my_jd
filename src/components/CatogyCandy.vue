<template>
    <div style="width:100%;height:100%;overflow: hidden">
        <div></div>
        <div></div>
        <ul class="phone_detail_container" v-if="$store.state.carts.cartsByCandy.length>0">
            <li v-for="list in $store.state.carts.cartsByCandy" class="phone_detail">
                <a>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            Req.getCartCandy()
                .then((resp)=>{
                    self.$store.dispatch('getCartCandy', resp.data)
                })
        }
    }
</script>
