<template>
    <div style="width:100%;height:100%;overflow: hidden">
        <div></div>
        <div></div>
        <ul class="phone_detail_container" v-if="$store.state.carts.cartsByHoildayBox.length>0">
            <li v-for="list in $store.state.carts.cartsByHoildayBox" class="phone_detail">
                <a>
...
<script>
    ...
    export default {
        name: "CatogyHoildaybox",
        created(){
            let self = this
            Req.getCartHoildayBox()
                .then((resp)=>{
                    self.$store.dispatch('getCartHoildayBox', resp.data)
                })
        }
    }
</script>
